...
describe('Server Routes', () => {
  ...
  describe('GET /pokemon - pokemon info page', () => {
    ...
    it('/pokemon?name=pokemonName should render pokemon image', async () => {
      const res = await request(app)
        .get('/pokemon?name=twinkles')
        .expect(200)
      expect(pokeStub.calledWith('twinkles'))
      const { window: { document } } = new JSDOM(res.text)
      const nameEl = document.querySelector(`img[alt="${POKEMON_DATA.name}"]`)
      console.log (nameEl)
      expect(nameEl.src).to.include(POKEMON_DATA.sprite)
    })
    it('/pokemon?name=pokemonName should render pokemon types', async () => {
      ...
      expect(pokeStub.calledWith('banana'))
      const { window: { document } } = new JSDOM(res.text)
      const nameEl = document.querySelector('[data-test-id="types"]')
      expect(nameEl.textContent).to.include('Types: ' + POKEMON_DATA.types.join(', '))
    })
    it('/pokemon?name=pokemonName should render pokemon height' , async () => {
      ...
      expect(pokeStub.calledWith('banana'))
      const { window: { document } } = new JSDOM(res.text)
      const nameEl = document.querySelector('[data-test-id="height"]')
      expect(nameEl.textContent).to.include(`Height: ${POKEMON_DATA.height}`)
    })
    it('/pokemon?name=pokemonName should render pokemon weight' , async () => {
      ...
      expect(pokeStub.calledWith('banana'))
      const { window: { document } } = new JSDOM(res.text)
      const nameEl = document.querySelector('[data-test-id="weight"]')
      expect(nameEl.textContent).to.include(`Weight: ${POKEMON_DATA.weight}`)
    })
